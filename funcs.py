        ...
        for iy in range(0, Ny):
            ...
            if not skewed:
                ...
                if ix < Nx - 1 or periodicX:
                    # To make sure only when both sites are within the altermanget
                    m = min(m_arr[iy, ix], m_arr[iy, (ix+1) % Nx])
...
def Delta_sc(gamma, D, Ui, T):
    ...
    Delta_i = Ui*np.sum(u_dn * np.conjugate(v_up) * f +
                        u_up * np.conjugate(v_dn) * (1 - f), axis=1)
    return Delta_i
...
def F_sc(gamma, D, V, T, Nx, Ny, bd):
    ...
    for ix in range(Nx):
        for iy in range(Ny):
            ...
            if ix < Nx-1:
                F[i, i + 1] = Vi[i] * np.sum(fd(D, T) * (np.conjugate(v_up[i])*u_dn[i+1] - u_up[i] * np.conjugate(
                    v_dn[i + 1])) + u_up[i] * np.conjugate(v_dn[i + 1]), axis=1)
                F[i + 1, i] = Vi[i] * np.sum(fd(D, T) * (np.conjugate(v_up[i+1])*u_dn[i] - u_up[i + 1]
                                             * np.conjugate(v_dn[i])) + u_up[i + 1] * np.conjugate(v_dn[i]), axis=1)

            if ix > 0:
                F[i, i - 1] = Vi[i] * np.sum(fd(D, T) * (np.conjugate(v_up[i]) * u_dn[i - 1] - u_up[i]
                                             * np.conjugate(v_dn[i - 1])) + u_up[i] * np.conjugate(v_dn[i - 1]), axis=1)
                F[i - 1, i] = Vi[i] * np.sum(fd(D, T) * (np.conjugate(v_up[i - 1]) * u_dn[i] -
                                             u_up[i - 1] * np.conjugate(v_dn[i])) + u_up[i - 1] * np.conjugate(v_dn[i]), axis=1)

            # if iy > 0:
            F[i, (i + Nx) % (Nx*Ny)] = Vi[i] * np.sum(f(D, T) * (np.conjugate(v_up[i])*u_dn[(i + Nx) % (Nx*Ny)] -
                                                                 u_up[i] * np.conjugate(v_dn[(i + Nx) % (Nx*Ny)])) + u_up[i] * np.conjugate(v_dn[(i+Nx) % (Nx*Ny)]), axis=1)
            F[(i + Nx) % (Nx*Ny), i] = Vi[i] * np.sum(f(D, T) * (np.conjugate(v_up[(i + Nx) % (Nx*Ny)])*u_dn[i] -
                                                                 u_up[(i + Nx) % (Nx*Ny)] * np.conjugate(v_dn[i])) + u_up[(i+Nx) % (Nx*Ny)] * np.conjugate(v_dn[i]), axis=1)

            F[i, i - Nx] = Vi[i] * np.sum(f(D, T) * (np.conjugate(v_up[i])*u_dn[i-Nx] - u_up[i]
                                          * np.conjugate(v_dn[i-Nx])) + u_up[i] * np.conjugate(v_dn[i-Nx]), axis=1)
            F[i - Nx, i] = Vi[i] * np.sum(f(D, T) * (np.conjugate(v_up[i-Nx])*u_dn[i] - u_up[i-Nx]
                                          * np.conjugate(v_dn[i])) + u_up[i-Nx] * np.conjugate(v_dn[i]), axis=1)
            # if iy < Ny-1:
    # -------------------------------------------------------------------
